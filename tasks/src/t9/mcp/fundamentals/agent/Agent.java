...
public class Agent {
    ...
    private void processToolCalls(List<ChatCompletionMessageToolCall> toolCalls, List<ChatCompletionMessageParam> messages) {
        for (ChatCompletionMessageToolCall toolCall : toolCalls) {
            ...
            // 1. Declare String toolResult (uninitialized — the try/catch below will assign it on all paths)
            // 2. Call mcpClient.callTool(functionName, arguments) in a try block, assign to toolResult
            // 3. In the catch block, build an error message string, print it, and assign to toolResult
            String toolResult = ""; // placeholder — remove and implement the try/catch above
